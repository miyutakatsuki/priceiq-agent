...
| 1:00–2:00 | **The failure (live)** | Streamlit "PVC Log" tab: side-by-side v1 vs v2 traces for "Should we discount sports gear by 10%?" | "Watch this. Same query, v1 prompt on the left, v2 on the right. v1's plan has only 3 tools. v2's plan has all 5. The agent succeeds either way — but v1's answer is missing the demand and weather context. We call this Shortcut Bias: the LLM took the minimum-effort path because nothing in the prompt insisted otherwise." |
| 2:00–2:45 | **Why this matters** | Streamlit "Final Answer" diff: v1 answer next to v2 answer | "v1 says 'discount viable, revenue up 21 to 46 percent.' v2 says the same — *plus* it surfaces that May seasonality is up 13%, Mother's Day is in 3 days, weather is mildly unfavorable. The v1 simulator didn't see these signals — it defaulted them to 1.0. The recommendation is correct in direction, but a stakeholder reading v1 has no idea those drivers exist." |
